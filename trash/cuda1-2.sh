...
context_types=("golden" "without")
contexts=("--use_context --no-irrelevant_context" "--no-use_context --irrelevant_context")
...
dataset_json_files=("bioasq_train_2000.json")
...
for model in "${models[@]}"; do # 1
    for k in "${!datasets[@]}"; do # 1
        ...
        for sample_suffix in "${context_types[@]}"; do # 2
